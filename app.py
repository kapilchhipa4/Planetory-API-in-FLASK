...
@app.route('/')
def hello_world():
    return jsonify(message='hello  world',abc='dkfjd')


@app.route('/super_simple')
def super_simple():
    return jsonify(message='Hello from the planetory api')
...
def parameters():   # you can send extran data like thaat  -->   ?name=kapil&age=23
...
def url_variables(name:str, age:int):
    if(age<18):       # ---> url_variables/kapil/23
        return jsonify(message='sorry '+name+' you are not old enough.'),401
    else:
        return jsonify(message='welcome '+name+' you are old enough')
...
def add_planet():
    ...
    if test:
        ...
        db.session.commit()
        return jsonify(message='you added to planet'), 201
...
def update_planet():
    ...
    planet = Planet.query.filter_by(planet_id = planet_id).first()
    if planet:  #it returns  addd if u change something and then commit changes will be reflected
